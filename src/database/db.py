...
def create_teacher(username, name, password):
    data={
        "username": username,"password":hash_pass(password),"name": name}
...
def get_student_attendence(student_id):
    response = supabase.table("attendence_logs").select("*").eq("student_id", student_id).execute()
    return response.data

def create_attendance(logs):

    try:
        print("Logs to insert:")
        print(logs)
